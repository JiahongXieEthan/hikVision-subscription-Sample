...
if (appKeyIndex === -1 || appKeyIndex === args.length - 1 || 
    ...
    console.error('使用方法: node test-query-subscription.js --appKey <your-app-key> --appSecret <your-app-secret>');
    process.exit(1);
}

const appKey = args[appKeyIndex + 1];
const appSecret = args[appSecretIndex + 1];
const url = 'https://labull.hsk.top:1443/artemis/api/eventService/v1/eventSubscriptionView';
...
const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
        try {
            ...
            // 格式化显示订阅信息
            if (jsonData.code === '0' && jsonData.data && jsonData.data.detail) {
                console.log('\n============ 订阅详情 ============');
                jsonData.data.detail.forEach((item, index) => {
                    console.log(`\n订阅 ${index + 1}:`);
                    console.log('  事件类型:', item.eventTypes);
                    console.log('  接收地址:', item.eventDest);
                });
            }
        } catch (e) {
